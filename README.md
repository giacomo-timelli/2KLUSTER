...
The TLS setup is based on the following components:

* **Deployment**: runs the actual application Pods.
* **Service**: exposes the Pods internally inside the cluster through a `ClusterIP`.
* **Ingress**: exposes the application externally and defines the HTTPS endpoint.
* **ClusterIssuer**: defines how certificates are requested from Let’s Encrypt.
* **Kubernetes Secret**: stores the certificate and private key generated by cert-manager.
...
After validation, the signed certificate is stored inside the referenced Kubernetes Secret. The NGINX Ingress Controller then uses this Secret to terminate TLS connections. When a user accesses the service through HTTPS, the Ingress handles the secure connection and forwards the request internally to the appropriate Kubernetes Service.
...
```text
groups claim → MinIO policy name
```
...
```json
{
  "groups": ["consoleAdmin", "minio"]
}
```

MinIO applies both the `consoleAdmin` policy and the `minio` policy. Permissions are additive, meaning that if a user belongs to multiple groups, the final permissions are the union of all associated policies.
...
Then apply the `ClusterIssuer` configuration:
